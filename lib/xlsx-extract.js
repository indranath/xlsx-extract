...
Cell.prototype.toTSV = function (tsv_float_comma, delimiter) {
	...
	delimiter = delimiter || options.tsv_delimiter || "\t";
	...
	if (this.val != null)
		switch (this.typ) {
			...
			case 'e':
				...
				break;
			default:
				console.log("Unknown cell type: \"%s\"", this.typ);
		}
};
...
Row.prototype.toTSV = function (tsv_float_comma, tsv_delimiter) {
	tsv_delimiter = tsv_delimiter || options.tsv_delimiter || "\t";
...
XLSXReader.prototype.parseXMLWorkbook = function (entry, workbook) {
	var parser = sax.createStream(true, {});
	parser.on('opentag', function(node) {
		name = node.name;
		attrs = node.attributes;
...
XLSXReader.prototype.parseXMLWorkbookRelations = function (entry, workbook) {
	var parser = sax.createStream(true, {});
	parser.on('opentag', function(node) {
		name = node.name;
		attrs = node.attributes;
...
XLSXReader.prototype.parseXML = function (cb) {
	...
	function parseSheet() {
		var lookup = "xl/worksheets/sheet" + caller.options.sheet_nr + ".xml";
		var sheet = undefined;
...
//				console.log('getting workbook info');
				caller.parseXMLWorkbook(entry, workbook);
			} else if (entry.path == "xl/_rels/workbook.xml.rels") {
//				console.log('getting workbook relation info');
				caller.parseXMLWorkbookRelations(entry, workbook);
			} else {
				entry.autodrain();
			}
		}).on('close', function () {
